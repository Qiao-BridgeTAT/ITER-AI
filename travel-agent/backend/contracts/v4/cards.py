...
class CardGenerationMetadata(V4ContractModel):
    ...
    generation_mode: Literal["qwen", "safe_seed_fallback", "provider_composed"]
    strategy_version: Literal["legacy", "attraction-v2", "dining-v2"] = "legacy"
...
class AttractionPreferenceCard(CardBase):
    ...
    def composition_has_city_and_personalized_directions(self) -> AttractionPreferenceCard:
        ...
        if self.generation_metadata.strategy_version == "attraction-v2":
...
class DiningPreferenceCard(CardBase):
    ...
    options: list[CardOption] = Field(min_length=5, max_length=5)
    ...
    def composition_has_two_local_directions(self) -> DiningPreferenceCard:
        ...
        if representative_count != 2:
            raise ValueError("dining preference card requires exactly two local directions")
        return self
...
class SpecificCandidateCard(CardBase):
    ...
    def count_composition_and_dispositions_match_domain(self) -> SpecificCandidateCard:
        city_led = (
            ...
            and self.generation_metadata.strategy_version == "attraction-v2"
        )
        expected = (
        ...
        if (
            ...
            and not (
                ...
                and self.generation_metadata.strategy_version == "dining-v2"
            )
            and self.status is not CardStatus.PARTIAL_AVAILABILITY
